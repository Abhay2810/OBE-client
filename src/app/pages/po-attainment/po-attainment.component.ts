    ...
    this.httpClient.get<{ poMap: any, finalCo: any, cos: CourseOutcomes[] }>(
      ...
      .then((res) => {
        ...
        this.totalPOPSOAttainment = poKeys.map((key, poIdx) => {
          let poKeyArray: string[] = Object.keys(res.poMap[key]);
          let poValueArray: number[] = Object.values(res.poMap[key]);
          let sumofCOMappingStrength: number = poValueArray.reduce((prev: any, next: any) => prev + next, 0)
          let mappingStrength = Number((sumofCOMappingStrength / poValueArray.length).toFixed(2));
          ...
          let coSumAvg = Number(poKeyArray.reduce((prev, next) => prev + coHrs[String(next)].coAvg, 0)) / poKeyArray.length;
